-- ============================================================
-- threadpool-isolation 부하 테스트 시드 데이터
-- ============================================================
...
--
-- FCM 토큰 전략:
--   'load_test_token_N' 형식의 가짜 고유 토큰을 삽입합니다.
--   Firebase는 유효하지 않은 토큰에 에러를 반환하지만, 서버는 N개 메시지를
--   빌드·전송 시도하므로 스레드 점유 부하 측정에는 충분합니다.
--
-- 멱등성: 이미 존재하는 멤버·토큰·구독은 중복 삽입되지 않습니다.
-- ============================================================

DROP PROCEDURE IF EXISTS seed_threadpool_isolation;

DELIMITER $$

CREATE PROCEDURE seed_threadpool_isolation(IN user_count INT)
BEGIN
...
-- ============================================================
CALL seed_threadpool_isolation(100);
